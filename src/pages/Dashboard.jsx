...
const Dashboard = ({ tasks, onToggleTask, isLoading, error }) => {
      ...
      <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-10 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <StatCard icon="🔥" label="روزهای متوالی" value="۱۲ روز" color="orange" />
            <StatCard icon="🏆" label="امتیاز کل" value="۲,۴۵۰" color="yellow" />
            <StatCard icon="🎯" label="اهداف فعال" value="۴" color="cyan" />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-4 bg-[#0d121d] border border-white/5 rounded-[2rem] p-8 h-fit">
                ...
                {["شنبه", "یکشنبه", "دوشنبه", "سه‌شنبه", "چهارشنبه", "پنج‌شنبه", "جمعه"].map((day, idx) => (
                  <div key={idx} className="space-y-2">
                    <div className="flex justify-between text-[10px] text-gray-500">
                      <span>{day}</span>
                      <span>{idx === 0 ? "۱۰۰٪" : idx === 2 ? "۷۰٪" : "۰٪"}</span>
                    </div>
                    <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                      <div className="h-full bg-[#00f2ea]" style={{ width: idx === 0 ? "100%" : idx === 2 ? "70%" : "0%" }}></div>
                    </div>
                  </div>
                ))}
              </div>
              <div className="mt-10 p-5 bg-[#00f2ea]/5 rounded-2xl border border-[#00f2ea]/10 text-center">
                ...
                <p className="text-2xl font-black text-[#00f2ea]">۶۴٪</p>
                <p className="text-[10px] text-green-400 mt-1">۱۲٪ رشد نسبت به قبل ✨</p>
              </div>
            </div>
          </div>
        </div>
